...
public class DayNightTransitioner : MonoBehaviour {
	...
	void Awake () {
		...
		float gameLength = GameObject.FindGameObjectWithTag("GameManager").GetComponent<GameData>().GetGameLength();
	...
	void Update () {
		...
		if(gameTimer > nightTime) {
			...
			overlayImage.color = Color.Lerp(overlayImage.color, colorTransitions[2], 0.003f);
		}
